...
export const PLUGIN_CATALOG: CatalogEntry[] = [
  ...
  // global equity engine settings live in the Equities settings tab.
  {
    status: 'ready',
    pricing: { model: 'free' },
    tagline: 'Commission-free US stocks & ETFs via Alpaca — installed by default.',
    config_schema: PLUGIN_CONFIG_SCHEMAS.alpaca,
    config_notes: 'Paper trading by default. Connect via OAuth or get keys at alpaca.markets → Paper Trading → API Keys.',
    manifest: {
      id: 'alpaca',
      kind: 'connector',
      version: '0.1.0',
      ...
      author: 'b1dz',
      description: 'Default broker. Commission-free US stocks & ETFs via Alpaca — OAuth or API keys, fractional/notional orders, IEX data (SIP via config), and a paper environment that mirrors live. No gateway to run.',
  ...
  },
  {
    ...
    config_notes: 'Heavyweight integration: requires running the IBKR Client Portal gateway and keeping its session alive. Set the gateway base URL. No simple paper flag — use an IBKR paper account.',
    manifest: {
      id: 'ibkr',
      kind: 'connector',
      version: '0.1.0',
      ...
      author: 'b1dz',
      description: 'US + international equities (LSE, TSE, XETRA, HKEX, TSX…) via the IBKR Client Portal Web API. Advanced: requires running an IBKR gateway session — not installed by default.',
