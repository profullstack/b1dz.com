/**
 * Seed a user's encrypted settings into user_settings.
 *
 * Usage:
 *   node scripts/seed-user-settings.mjs
 *
 * Reads SUPABASE_URL, SUPABASE_SECRET_KEY, SETTINGS_ENCRYPTION_KEY from env.
 * Encrypts secrets with AES-256-GCM (same algorithm as browser-crypto.ts).
 */
...
if (!SUPABASE_URL || !SUPABASE_SECRET || !ENC_KEY_B64) {
  console.error('Missing: SUPABASE_URL / SUPABASE_SECRET_KEY / SETTINGS_ENCRYPTION_KEY');
  process.exit(1);
}

// ── Secrets to store (encrypted) ────────────────────────────────────────────
...
if (process.env.ZEROX_API_KEY) plain.ZEROX_API_KEY = process.env.ZEROX_API_KEY;

// ── AES-256-GCM (matches browser-crypto.ts) ──────────────────────────────────
...
// Merge secrets: decrypt existing cipher, then merge new secrets on top
let existingSecrets = {};
if (existing?.payload_secret_ciphertext && existing.payload_secret_iv && existing.payload_secret_tag) {
  try {
    ...
    console.warn('Could not decrypt existing cipher:', e.message);
  }
}
...
console.log(`Saving ${Object.keys(mergedSecrets).length} secrets, ${Object.keys(mergedPlain).length} plain fields`);
console.log('Secrets:', Object.keys(mergedSecrets).join(', '));
